...
Càng có nhiều không gian để một mô hình đưa ra quyết định tự chủ thì tác động khi nó gặp sự cố càng lớn, do đó cần có các ràng buộc, cơ chế xác minh và sửa chữa phức tạp hơn để đảm bảo độ tin cậy. Ưu điểm thực sự của các nhà sản xuất mô hình không phải là "làm cho khung mỏng hơn", mà là tối ưu hóa mô hình và dây nịt ngoại vi một cách hợp tác và tiếp tục lặp lại.

#### Cơ chế học tập của Agent: post-training, In-Context Learning (học trong ngữ cảnh) và học từ bên ngoài

Trước đây chúng ta đã thảo luận về cách mô hình nội hóa các lệnh gọi công cụ thành các khả năng gốc thông qua học tăng cường. Nhưng việc học của Agent không chỉ diễn ra ở giai đoạn huấn luyện - một số độc giả cho rằng mô hình phải được huấn luyện khi Agent học hỏi kinh nghiệm. Trên thực tế, post-training không phải là cách duy nhất Agent học hỏi kinh nghiệm. Cơ chế học tập của Agent có thể được tóm tắt thành ba mô hình bổ sung (Hình 1-1):

![Hình 1-1: Ba mô hình học tập của Đặc vụ ](images_vi/fig1-1.svg)
...
- **Tool Definitions**(Định nghĩa công cụ): khai báo tên, mô tả chức năng và định dạng tham số của các công cụ có sẵn Agent. Nếu không có định nghĩa công cụ, Agent không thể nhận dạng và gọi bất kỳ công cụ nào - thí nghiệm cắt bỏ (Thí nghiệm 1.1) sẽ xác minh điều này. Định nghĩa công cụ và system prompt cùng nhau tạo thành **tiền tố tĩnh** không thay đổi trong cuộc hội thoại.
...
- **Trả lời mẫu**(Tin nhắn hỗ trợ): Các câu trả lời do mô hình tạo trước đây chứa tối đa ba phần - quá trình suy nghĩ (lý luận, tức là chuỗi suy nghĩ nội bộ, duy trì sự mạch lạc trong suy nghĩ và khả năng diễn giải khi ra quyết định), nội dung văn bản (nội dung, tức là trả lời người dùng) và yêu cầu gọi công cụ (tool_calls, tức là cách Agent thực hiện hành động). Trong một câu trả lời cụ thể, cả ba không nhất thiết phải xuất hiện cùng lúc: ví dụ Agent thường chỉ có lý luận + tool_calls khi quyết định gọi một công cụ và thường chỉ có lý luận + nội dung khi đưa ra câu trả lời cuối cùng.
...
![Hình 1-2: Thí nghiệm 1.1—Thiết kế thử nghiệm cắt bỏ ngữ cảnh ](images_vi/fig1-2.svg)
...
![Hình 1-3: Trajectory tác nhân - Vòng lặp ReAct của nhiệm vụ tóm tắt đa tiền tệ ](images_vi/fig1-3.svg)
...
Thử nghiệm này thể hiện khả năng Agent vốn có của **Kimi K3** và thể hiện mô hình mới của "mô hình là Agent". Kimi K3 được Moonshot AI phát hành vào năm 2026. Đây là mô hình Mixture of Experts (MoE, Mixture of Experts) với khoảng 2,8 nghìn tỷ thông số - bạn có thể coi MoE như một nhóm chuyên gia: đối mặt với nhiều loại câu hỏi khác nhau, hệ thống sẽ tự động chọn ra các chuyên gia phù hợp nhất để trả lời mà không cần tất cả các chuyên gia đều có mặt tại hiện trường cùng lúc, điều này không chỉ đảm bảo năng lực mà còn nâng cao hiệu quả. Nó có cửa sổ ngữ cảnh gồm 1 triệu mã thông báo, khả năng hiểu trực quan gốc và "chế độ suy nghĩ" luôn bật; thông qua đào tạo học tăng cường, mô hình này nội hóa khả năng gọi công cụ thành khả năng gốc và có thể tự động đưa ra quyết định cũng như thực hiện các tác vụ như tìm kiếm trên web.

Các quan sát chính bao gồm: mô hình học cách sử dụng các công cụ một cách tự nhiên thông qua đào tạo RL mà không cần các lớp điều phối bổ sung; mô hình tự quyết định khi nào cần tìm kiếm và tìm kiếm cái gì, thể hiện quyền tự chủ thực sự; nó có thể linh hoạt điều chỉnh các chiến lược dựa trên kết quả tìm kiếm và xác định độc lập xem thông tin có đầy đủ hay không; Khả năng sử dụng các công cụ không được "dạy" cho mô hình mà được học thông qua các tương tác lặp đi lặp lại với môi trường.
...
![Hình 1-4: Kiến trúc "Model is Agent" - lệnh gọi công cụ gốc ](images_vi/fig1-4.svg)
...
![Hình 1-5: Vòng lặp thực thi của Tác nhân tự trị ](images_vi/fig1-5.svg)
...
![Hình 1-6: Giao diện soạn thảo quy trình công việc n8n ](images_vi/n8n-workflow.png)
...
4. ★★ Việc thiếu “phản hồi kết quả công cụ” trong thí nghiệm cắt bỏ đã khiến Agent rơi vào một vòng lặp vô hạn. Trong môi trường sản xuất, ngoài kết quả công cụ bị thiếu, tình huống nào khác có thể gây ra vòng lặp Agent? Bạn sẽ thiết kế cơ chế phát hiện và chấm dứt nào?
